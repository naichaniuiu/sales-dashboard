# -*- coding: utf-8 -*-
"""
将回款周期下钻数据注入到 generate_html.py 生成的HTML中
步骤：插入数据变量 → 替换renderSalesCycle → 添加renderCustCycle/backToSalesCycle → 更新closeSalesModal
"""
import sys
...
# 4. Add renderCustCycle and backToSalesCycle after closeSalesModal function
# (works with both old and new HTML generated by generate_html.py)
insert_point = html.find('function closeSalesModal()')
if insert_point == -1:
    print('ERROR: closeSalesModal not found')
    sys.exit(1)

# Find end of closeSalesModal function
brace_count = 0
func_end_pos = -1
...
print(f'closeSalesModal ends at: {func_end_pos}')
...
html = html[:func_end_pos] + new_funcs + html[func_end_pos:]
print('Added renderCustCycle and backToSalesCycle')

# 5. Update closeSalesModal to reset cycle back state
old_close_simple = "    document.getElementById('salesModal').style.display = 'none';\n}"
new_close_simple = "    document.getElementById('salesModal').style.display = 'none';\n    window._salesCycleBack = null;\n}"

old_close_full = "    window._salesDebtBack = null;\n    window._salesPerfBack = null;\n}"
new_close_full = "    window._salesDebtBack = null;\n    window._salesPerfBack = null;\n    window._salesCycleBack = null;\n}"

if old_close_full in html:
    html = html.replace(old_close_full, new_close_full)
    print('Updated closeSalesModal (full version)')
elif old_close_simple in html:
    html = html.replace(old_close_simple, new_close_simple)
    print('Updated closeSalesModal (simple version)')
else:
    print('WARNING: closeSalesModal pattern not found')

with open('中西部大区26财年Q1数据看板.html', 'w', encoding='utf-8') as f:
    f.write(html)

print('\nDone! HTML updated with cycle drill-down successfully.')
